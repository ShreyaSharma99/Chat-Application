...
class TCPClient extends Cryptography { 
    ...
    public static void send_register_message(String username, boolean if_to_receive, DataOutputStream out_to_server) throws IOException{
        ...
        if(!if_to_receive)
            ...
            message = "REGISTER TORECV "+ username + " Key: " + pub_key+"\n";
...
class SocketThread extends TCPClient implements Runnable { 
    ...
    public SocketThread(Socket connectionSocket, BufferedReader inFromClient, DataOutputStream outToServer, BufferedReader inputServer) {
        this.connectionSocket = connectionSocket;
        this.input = inFromClient;
        this.output = outToServer;
        this.input_from_server = inputServer;
        this.output_to_server = outToServer;
        if_receiving = false;
     } 

     public SocketThread(Socket connectionSocket, BufferedReader inFromServer, DataOutputStream outToClient, DataOutputStream outputServer) {
        this.connectionSocket = connectionSocket;
        this.input = inFromServer;
        this.output = outToClient;
        this.output_to_server = outputServer;
        this.input_from_server = inFromServer;
        if_receiving = true;
     }
    ...
    public void send_message(String username, long content_length, String message, String key1) throws Exception{
        ...
        //message_to_send += message_encrypted + "\n"; 
         message_to_send += message_encrypted + "\n" + hash_base64 +"\n";
        output_to_server.writeBytes(message_to_send + "\n");
    }
    ...
    public boolean check_if_error(String message){
        String[] words = message.split(" ");
        if(words[0].equals("ERROR"))
            return true;
        return false;
        ...
        while(true) { 
            try {
                ...
                long content_length = 0;
                    ...
                    output_to_server.writeBytes(fetch_key);
                    String ser_message = input_from_server.readLine();
                    if(check_if_error(ser_message)){
                        String blank = input_from_server.readLine();
                        respond_to_server_response(ser_message, recipient_username);
                    }
                    else{
    ...
    public boolean check_if_same(byte[] arr1, byte[] arr2){
    ...
    public void read_inp_from_server(){
        while(true) { 
            try {
                ...
                boolean message_started = false;
                long content_length = 0;
                while(this.input_from_server.ready()){
                    ...
                    if(!message_started){
                        ...
                        if(words[0].equals("FORWARD")){
                            ...
                            //System.out.println("we got it "+ sender_username);
                        }
                        ...
                        //System.out.println("we got "+ sender_username);
                        ...
                        //System.out.println(decrypted_hash.length +" " + hash_digested.length);
                        if(((long)message.length() == content_length) && check_if_same(decrypted_hash, hash_digested)){
                            System.out.println("Message from "+ sender_username +": " + message);
